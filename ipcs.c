...
static int dump_shared_memory(int, ulong, int);
static int dump_semaphore_arrays(int, ulong, int);
static int dump_message_queues(int, ulong, int);
...
cmd_ipcs(void)
{
	...
	while ((c = getopt(argcnt, args, "smMq")) != EOF) {
		switch(c) {
			case 's':
				sem = 1;
				break;
	...
	if (!value_index) {
		if (shm)
			dump_shared_memory(specified, 0, verbose);
		if (sem)
			dump_semaphore_arrays(specified, 0, 0);
		if (msg)
			dump_message_queues(specified, 0, 0);
	} else {
		open_tmpfile();
		i = 0;
		while (i < value_index) {
...
char *help_ipcs[] = {
	...
	"[-smMq] [id | addr]",                 /* argument synopsis, or " " if none */
